...
def nameClear(name:str,productLink):#helper function to clear the name of the product
...
async def search(q, host):
    ...
    webPageContents = webPageContents.replace(
        'style="color:#000000;font-size:14px;font-style:normal;font-weight:700">₹', '>Rs. '
    )
    ...
    while i < len(products):
        try:
            ...
            if priceCheck.split('</option>'):
                try:
                    ...
                    try:
                        ...
                        for thumb in thumbnails_arr:
                            ...
                            if 'jpeg' in possible_thumbnail or 'image' in possible_thumbnail:
                ...
                if productName in ("", None):
                    try:
                        ...
                        if len(linkDetails) == 1:
                            ...
                            print("Failed to obtain product name and link from Method B")
                        ...
                        if reversion:
                            ...
                            reversion = False
                            method = 'D'
                            print("Failed to obtain product name and link from Method C")
                    except Exception as e:
                        print(e)
                    if productName in ("", None):
                        print("Failed to obtain product name and link from known methods")
                    else:
                        print("Successfully obtained product name and link from known methods")
                        try:
                            if thumbnail is None:
                                thumbnail = webPageContents.split(f'alt="{productName}"')[1].split('src="')[1].split('"')[0]
                        except Exception:
                            thumbnail = None
                        if i + 1 != len(products):
                            nextItem = products[i + 1].split('</div>')[0].replace(',', '').split('<!-- -->')
                        ...
                        result.append({
                            ...
                            "name":nameClear(productName.replace('&#x27;', "'"),clean(productLink)),
                            "link": clean(productLink),
                            "current_price": currentPrice,
                            "original_price": originalPrice,
                            "discounted": isDiscounted,
                            "thumbnail": thumbnail,
                            "query_url": clean(productLink).replace('www.flipkart.com', f'{host}/product').replace('dl.flipkart.com', f'{host}/product')
                        })
                else:
                    try:
                        if thumbnail is None:
                            ...
                            if len(thumbnail) == 1:
                                thumbnail = webPageContents.split(f'alt="{productName[:5]}')
                    ...
                    result.append({
                        ...
                        "name":nameClear(productName.strip(),clean(productLink).replace('http://', 'https://')),
                ...
                print("Ignoring amount", currentPrice, ": Suspected to be dropdown menu item")
